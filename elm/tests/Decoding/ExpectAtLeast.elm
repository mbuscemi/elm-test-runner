module Decoding.ExpectAtLeast exposing (parsesCorrectly)
...
parsesCorrectly =
    ...
    describe "parsesCorrectly" <|
        ...
        , test "has correct message" <|
            \_ ->
                Expect.equal (Failure.getMessage failure) "Expect.atLeast"
        , test "has no given" <|
            \_ ->
        ...
        , test "has expected actual text" <|
            \_ ->
                Expect.equal (Failure.getActual failure) "1"
...
failingJsonString =
    """{"event":"testCompleted","status":"fail","labels":["FailureOutputValidation","Expect.atLeast","basic case","failing on less"],"failures":[{"given":null,"message":"Expect.atLeast","reason":{"type":"custom","data":{"first":"2","second":"1","comparison":"Expect.atLeast"}}}],"duration":"12"}"""
